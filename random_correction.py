		...
		if folder not in folder_prefix.keys():
			...
			for name in os.listdir('input/%s' % folder):
				...
				if bool(re.search('somatic.snv_mnv.vcf.gz$', name)): ### checks if snv_mnv.vcf.gz in the iterated variable
					...
					### THIS was old bootstrapping lines. ###
					# for n,randombed in enumerate(os.listdir('./final/randombeds')):
					# 	sys.stdout.write("\r{0}".format(randombed))
					# 	sys.stdout.flush()
					...
					# 		#print 'random_no', random_mutation_count[0]
					# 		random_counter += 1
					# 	snv_log.write(str(random_mutation_count) + '\t')
					# 	snv_log.flush()
					random_counter=BEDbootstrap(logfilename = snv_log,SNV=True,verbose=True)
					
					# if random_counter == 0:
					# 	snv_log.write('FDR:'+ str(1) + '\t')
					# else:
					snv_log.write('FDR:\t' + str(float(random_counter) / float(randombedcount)) + '\t')
					...
					# 	bedtoolspath,'intersect', '-a' ,"-",'-b','./final/randombeds/'+ randombed, '|' ,'wc']
					# 	random_mutation_count=subprocess.check_output(' '.join(random_isolation_bash_script),shell=True)
					# 	random_mutation_count=int(random_mutation_count.split()[0])
					# 	if float(count[0]) <= float(random_mutation_count):
					# 		random_counter += 1
					# 		#print 'indel'
					# 		#print 'count',count[0]
					# 		#print 'random_no', random_mutation_count[0]
					# 		
					# 	indel_log.write(str(random_mutation_count) + '\t')
					#
					random_counter=BEDbootstrap(logfilename = indel_log,SNV=False,verbose=True) 
					
					# if random_counter == 0:
					# 	indel_log.write('FDR'+':'+str(1) + '\t')
					# else:
					indel_log.write('FDR:\t'+ str( float(random_counter) / float(randombedcount) ) + '\t')
					indel_log.flush()
